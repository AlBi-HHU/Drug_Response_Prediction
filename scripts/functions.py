...
def ensp_to_hugo_map():
    protein_info = pd.read_csv('DepMap_Data/TGSA_Data/9606.protein.info.v11.0.txt', delimiter='\t')
...
def get_STRING_graph(threshold, gene_list):
    ...
    if not os.path.exists(save_path):
        ...
        edge_index = []
        with open('DepMap_Data/TGSA_Data/protein.links.detailed.v11.0.txt', 'r') as f: # get this 400 GB file from https://string-db.org/cgi/download.pl and then store it in DepMap_Data/TGSA_Data/
            next(f)
                ...
                if combined_score > threshold1000:
                    if node0 in ensp_map.keys() and node1 in ensp_map.keys():
                        ...
                        if gene0 in gene_list and gene1 in gene_list:
                            ...
                            print(i, combined_score, node0, node1, gene0, gene1, gene_list.index(gene0), gene_list.index(gene1))
...
# adapted from TGDRP/TGSA
def get_predefine_cluster(edge_index, threshold, device):
    save_path = f'features/cluster_predefine_PPI_{threshold}.npy'
    if not os.path.exists(save_path):
        with open('features/cosmic_cancer_related_genes_gdsc.json', 'r') as f:
...
def get_knn(x_cell, x_drug, y, knn, inputs):
    ...
    y = y.loc[cell_lines]
    with open('features/cosmic_cancer_related_genes_gdsc.json', 'r') as f:
        cosmic_cancer_related_genes_gdsc = json.load(f)

    exp = pd.read_csv('features/cell_features/exp_tpm.csv', index_col=0, low_memory=False)
    cosmic_cancer_related_genes_gdsc_j = [c + '.exp' for c in cosmic_cancer_related_genes_gdsc]
    ...
    exp = exp.loc[:, cosmic_cancer_related_genes_gdsc_j]
...
def process_inputs_and_output(inputs, solver):
    ...
    if solver == 'MMLP' or solver == 'MMLPcosmic' or solver == 'MMLPid':
        if solver == 'MMLPcosmic': #the following code is for testing on COSMIC genes only (those that are used by TGDRP/TGSA and appear in the GDSC2 dataset)
            ...
            for j, i in enumerate(inputs_cell_list):
                ...
                if 'cnv' in i:
                    cosmic_cancer_related_genes_gdsc_j = [c + '.cnv' for c in cosmic_cancer_related_genes_gdsc]
                elif 'exp' in i:
                    cosmic_cancer_related_genes_gdsc_j = [c + '.exp' for c in cosmic_cancer_related_genes_gdsc]
                else: # if 'mut' in i:
                    cosmic_cancer_related_genes_gdsc_j = [c + '.mut' for c in cosmic_cancer_related_genes_gdsc]
                x_cell_i = x_cell_i.loc[cell_lines, cosmic_cancer_related_genes_gdsc_j]
    ...
    if solver == 'TGDRP' or solver == 'TGSA' or solver == 'TGDRPid' or solver == 'TGSAid':
        ...
        with open('features/cosmic_cancer_related_genes_gdsc.json', 'r') as f:
            cosmic_cancer_related_genes_gdsc = json.load(f)
        edge_index = get_STRING_graph(threshold, cosmic_cancer_related_genes_gdsc)
        edge_index = torch.tensor(edge_index, dtype=torch.long)
        cn_ex_mu = []
        ...
        for i in inputs_cell_list:
            ...
            x_cell_i = x_cell_i.loc[cell_lines, cosmic_cancer_related_genes_gdsc]
